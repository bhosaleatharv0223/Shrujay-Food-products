...
export function InvoicePreview({ invoice, onDownload }: Props) {
  return (
    <div className="rounded-[24px] border border-[#E4D2B4] bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={onDownload} className="flex items-center justify-center gap-2 rounded-2xl bg-[#6B4226] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#8B5E3C]">
          <Download size={16} /> Download
        </button>
      </div>

      <div className="rounded-[24px] border border-[#E4D2B4] bg-[#FFF9F0] p-4 text-sm text-[#5A3822]">
        <div className="mb-3 flex flex-col gap-2 border-b border-[#E4D2B4] pb-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
